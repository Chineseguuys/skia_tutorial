...
void draw1(SkCanvas* canvas) {
    SkIRect bounds;
    source.getBounds(&bounds);
    spdlog::info("{}: source bounds {}x{} -> {}x{}", __func__, bounds.fLeft, bounds.fTop, bounds.fRight, bounds.fBottom);
    bounds.inset(100, 100);
    // 向图像的中心区域收缩 100 个像素
    spdlog::info("{}: source bounds after insert {}x{} -> {}x{}", 
            __func__, bounds.fLeft, bounds.fTop, bounds.fRight, bounds.fBottom);
    SkBitmap bitmap;
    source.extractSubset(&bitmap, bounds);
    spdlog::info("{}: bitmap wxh = {}x{}", __func__, bitmap.width(), bitmap.height());
    canvas->scale(.5f, .5f);
