...
enum {
    op_nand = region_operator<ui::Rect>::op_nand,
    op_and  = region_operator<ui::Rect>::op_and,
    op_or   = region_operator<ui::Rect>::op_or,
    op_xor  = region_operator<ui::Rect>::op_xor
};
...
bool Region::hasSameRects(const Region& other) const {
    ...
    ui::Rect const* thisRects = getArray(&thisRectCount);
    size_t otherRectCount = 0;
    ui::Rect const* otherRects = other.getArray(&otherRectCount);
