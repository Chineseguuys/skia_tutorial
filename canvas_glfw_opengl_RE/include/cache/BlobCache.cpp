...
#endif
    spdlog::info("initializing random seed using {}", (unsigned long long)now);
}
...
BlobCache::InsertResult BlobCache::set(const void* key, size_t keySize, const void* value,
    ...
    if (mMaxKeySize < keySize) {
        spdlog::trace("set: not caching because the key is too large: {} (limit: {})", keySize, mMaxKeySize);
        return InsertResult::kKeyTooBig;
    }
    if (mMaxValueSize < valueSize) {
        spdlog::trace("set: not caching because the value is too large: {} (limit: {})", valueSize, mMaxValueSize);
        return InsertResult::kValueTooBig;
    }
    if (mMaxTotalSize < keySize + valueSize) {
        spdlog::trace("set: not caching because the combined key/value size is too "
        "large: {} (limit: {})", keySize + valueSize, mMaxTotalSize);
        return InsertResult::kCombinedTooBig;
    }
    if (keySize == 0) {
        spdlog::trace("set: not caching because keySize is 0");
        return InsertResult::kInvalidKeySize;
    }
    if (valueSize == 0) {
        spdlog::trace("set: not caching because valueSize is 0");
        ...
        if (index == mCacheEntries.end() || cacheEntry < *index) {
            ...
            if (mMaxTotalSize < newTotalSize) {
                if (isCleanable()) {
                    ...
                    spdlog::trace("set: not caching new key/value pair because the "
                        "total cache size limit would be exceeded: {} "
                        "(limit: {})",
            ...
            spdlog::trace("set: created new cache entry with {} byte key and {} byte value", keySize, valueSize);
        } else {
            ...
            if (mMaxTotalSize < newTotalSize) {
                if (isCleanable()) {
                    ...
                    spdlog::trace("set: not caching new value because the total cache "
                    "size limit would be exceeded: {} (limit: {})",
                    keySize + valueSize, mMaxTotalSize);
                    return InsertResult::kNotEnoughSpace;
                }
            }
            ...
            mTotalSize = newTotalSize;
            spdlog::trace("set: updated existing cache entry with {} byte key and {} byte "
            "value",
            keySize, valueSize);
        }
...
size_t BlobCache::get(const void* key, size_t keySize, void* value, size_t valueSize) {
    if (mMaxKeySize < keySize) {
        spdlog::trace("get: not searching because the key is too large: {} (limit {})", keySize,
              mMaxKeySize);
        return 0;
    }
    ...
    auto index = std::lower_bound(mCacheEntries.begin(), mCacheEntries.end(), cacheEntry);
    if (index == mCacheEntries.end() || cacheEntry < *index) {
        spdlog::trace("get: no cache entry found for key of size {}", keySize);
        return 0;
    }
    ...
    if (valueBlobSize <= valueSize) {
        spdlog::trace("get: copying {} bytes to caller's buffer", valueBlobSize);
        memcpy(value, valueBlob->getData(), valueBlobSize);
    } else {
        spdlog::trace("get: caller's buffer is too small for value: {} (needs {})", valueSize,
              valueBlobSize);
    }
    spdlog::info("BlobCache::get key size {}, data size {}", keySize, valueBlobSize);
...
int BlobCache::flatten(void* buffer, size_t size) const {
    ...
    if (size < sizeof(Header)) {
        spdlog::error("flatten: not enough room for cache header");
        return 0;
    }
    ...
    for (const CacheEntry& e : mCacheEntries) {
        ...
        if (byteOffset + totalSize > size) {
            spdlog::error("flatten: not enough room for cache entries");
            return -EINVAL;
        }
...
int BlobCache::unflatten(void const* buffer, size_t size) {
    ...
    if (size < sizeof(Header)) {
        spdlog::error("unflatten: not enough room for cache header");
        return -EINVAL;
    }
    const Header* header = reinterpret_cast<const Header*>(buffer);
    if (header->mMagicNumber != blobCacheMagic) {
        spdlog::error("unflatten: bad magic number: {}", header->mMagicNumber);
        return -EINVAL;
    }
    std::string buildId = "ro.build.id";
    ...
    for (size_t i = 0; i < numEntries; i++) {
        if (byteOffset + sizeof(EntryHeader) > size) {
            clear();
            spdlog::error("unflatten: not enough room for cache entry headers");
            return -EINVAL;
        }
