...
bool ShaderCache::validateCache(const void* identity, ssize_t size) {
    ...
    if (nullptr == identity || size < 0) {
        ...
        //     spdlog::warn("ShaderCache::validateCache invalid cache identity");
        // }
        mBlobCache->clear();
        return false;
    }
    ...
    // if (CC_UNLIKELY(Properties::debugLevel & kDebugCaches)) {
    //     spdlog::warn("ShaderCache::validateCache cache validation fails");
    // }
    spdlog::warn("ShaderCache::validateCache cache validation fails");
    mBlobCache->clear();
    return false;
...
sk_sp<SkData> ShaderCache::load(const SkData& key) {
    ...
    spdlog::info("ShaderCache::load");
    // bt::StackTracer __stack_trace__{};
    ...
    if (valueSize > mObservedBlobValueSize) {
        spdlog::error("ShaderCache::load value size is too big {}", (int)valueSize);
        free(valueBuffer);
        return nullptr;
    }
...
void ShaderCache::set(const void* key, size_t keySize, const void* value, size_t valueSize) {
    spdlog::info("ShaderCache::set key size {}, data size {}", keySize, valueSize);
...
void ShaderCache::store(const SkData& key, const SkData& data, const SkString& /*description*/) {
    ...
    spdlog::info("ShaderCache::store");
    // bt::StackTracer __stack_tracer__{};
    std::lock_guard lock(mMutex);
    mNumShadersCachedInRam++;
    // ATRACE_FORMAT("HWUI RAM cache: %d shaders", mNumShadersCachedInRam);
    ...
    if (keySize == 0 || valueSize == 0 || valueSize >= maxValueSize) {
        spdlog::warn("ShaderCache::store: sizes {} {} not allowed", (int)keySize, (int)valueSize);
        return;
    }
