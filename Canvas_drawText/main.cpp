...
void draw(SkCanvas* canvas) {
    SkPaint paint;
    SkFont defaultFont = SkFont(typeFace);
...
// https://fiddle.skia.org/c/@Canvas_drawTextBlob_2
void draw1(SkCanvas* canvas) {
    SkTextBlobBuilder textBlobBuilder;
    SkFont font = SkFont(typeFace, 50);
    const SkTextBlobBuilder::RunBuffer& run =
            textBlobBuilder.allocRun(font, 3, 20, 100);
    run.glyphs[0] = 20;
    run.glyphs[1] = 21;
    run.glyphs[2] = 22;
...
void draw2(SkCanvas* canvas) {
    ...
    const SkTextBlobBuilder::RunBuffer& run =
            textBlobBuilder.allocRun(font, 3, 20, 100);
    SkSpan<SkGlyphID> _glyphs{run.glyphs, 3};
    // 将 text 转为 SkGlyphID 数组
    typeFace->textToGlyphs("abc", strlen("abc"), SkTextEncoding::kUTF8, _glyphs);
    sk_sp<const SkTextBlob> blob = textBlobBuilder.make();
    SkPaint paint;
    paint.setColor(SK_ColorBLUE);
    canvas->drawTextBlob(blob.get(), 0, 0, paint);
...
// https://fiddle.skia.org/c/@Canvas_drawTextRSXform
