...
static const int DRAW_HEIGHT = 20;
...
void draw(SkCanvas* canvas) {
    SkBitmap bitmap;
    // 注意这里的 bgr 和 rgb 排列的差异
    bitmap.setInfo(SkImageInfo::Make(1, 1, kBGRA_8888_SkColorType, kOpaque_SkAlphaType));
    bitmap.allocPixels();
    bitmap.eraseColor(SK_ColorRED);
    spdlog::info("we get pixel raw data with color red = {:x}", *(SkPMColor*) bitmap.getPixels());
    canvas->scale(16, 16);
    *(SkPMColor*) bitmap.getPixels() = SkPreMultiplyColor(SK_ColorRED);
    ...
    *(SkPMColor*) bitmap.getPixels() = SkPreMultiplyColor(SK_ColorGREEN);
    spdlog::info("premulticolor for red = {:x}, for blue = {:x}, for green = {:x}", 
            SkPreMultiplyColor(SK_ColorRED), 
            SkPreMultiplyColor(SK_ColorBLUE),
            SkPreMultiplyColor(SK_ColorGREEN)
    );
...
void draw1(SkCanvas* canvas) {
    SkBitmap bitmap;
    SkImageInfo imageInfo = SkImageInfo::Make(1, 1, kBGRA_8888_SkColorType, kOpaque_SkAlphaType);
    ...
    canvas->scale(16, 16);
    canvas->drawImage(bitmap.asImage(), 0, 0);
    *(SkPMColor*)bitmap.getPixels() = SkPreMultiplyColor(SK_ColorYELLOW);
    // bitmap.notifyPixelsChanged(); // will crash for check(!this->isImmutable())
    canvas->drawImage(bitmap.asImage(), 2, 0);
    *(SkPMColor*) bitmap.getPixels() = SkPreMultiplyColor(SK_ColorGREEN);
    canvas->drawImage(bitmap.asImage(), 4, 0);
}
