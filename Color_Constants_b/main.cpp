...
// https://fiddle.skia.org/c/@Color_Constants_b
void draw(SkCanvas* canvas) {
    ...
    // 将区域 {24,24,192,192} 设置为红色
    pixmap.erase(SK_ColorRED, {24,24,192,192});
    // 将区域 {48,48,168,168} 设置为透明的
    pixmap.erase(SK_ColorTRANSPARENT, {48, 46,168,168});
    SkBitmap bitmap;
...
// https://fiddle.skia.org/c/@Color_Constants_c
void draw1(SkCanvas* canvas) {
    ...
    // 将区域 {24,24,192,192} 设置为红色
    pixmap.erase(SK_ColorRED, {24,24,192,192});
    // 将区域 {48,48,168,168} 设置为黑色的
    pixmap.erase(SK_ColorBLACK, {48, 46,168,168});
    SkBitmap bitmap;
    bitmap.installPixels(pixmap);
    canvas->drawImage(bitmap.asImage(), 0, 0);
    canvas->drawImage(bitmap.asImage(), 48,48);
...
// https://fiddle.skia.org/c/@Color_Constants_d
void draw2(SkCanvas* canvas) {
    ...
    pixmap.erase(SK_ColorRED, {24,24,192,192});
    // 将区域 {48,48,168,168} 设置为黑色的
    pixmap.erase(SK_ColorWHITE, {48, 46,168,168});
    SkBitmap bitmap;
    bitmap.installPixels(pixmap);
    canvas->drawImage(bitmap.asImage(), 0, 0);
    canvas->drawImage(bitmap.asImage(), 48,48);
