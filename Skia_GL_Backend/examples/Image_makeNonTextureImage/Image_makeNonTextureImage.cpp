...
class Image_makeNonTextureImage : public Effect {
public:
    Image_makeNonTextureImage() : Effect() {}
    ...
    void draw(SkCanvas* canvas) override {
        ...
        auto drawImage = [=](sk_sp<SkImage> image, const char* label) -> void {
            ...
            spdlog::info("[{}:{}]:NonTextureImage isTextureBacked = {}, is valid = {}",
                __FUNCTION__, __LINE__,
                nonTexture->isTextureBacked(),
                nonTexture->isValid(dContext->asRecorder())
            );
            canvas->drawImage(nonTexture, 0, 0);
            canvas->drawString(label, 20, nonTexture->height() >> 2, font, paint);
        };
...
extern "C" Effect* createEffect() {
    return new Image_makeNonTextureImage{};
}