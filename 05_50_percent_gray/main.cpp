...
static const int DRAW_WIDTH = 600;
static const int DRAW_HEIGHT = 200;
...
int main(int argc, char* argv[]) {
    ...
    // clear the canvans
    recordingCanvas->drawColor(SK_ColorWHITE);

    // BW Dither
    recordingCanvas->translate(5,5);
    SkPaint paint;
    paint.setShader(makeBwDither());
    recordingCanvas->drawRect({0, 0, 100, 100}, paint);
    SkPaint black;
    recordingCanvas->drawString("BW Dither", 0, 125, font, black);
    
    // Scaled BW Dither
    recordingCanvas->translate(105, 0);
    recordingCanvas->save();
    recordingCanvas->scale(0.5, 0.5);
    recordingCanvas->drawRect({0, 0, 200, 200}, paint);
    recordingCanvas->restore();
    paint.setShader(nullptr);
    recordingCanvas->drawString("Scaled Dither", 0, 125, font, paint);
    ...
    recordingCanvas->drawRect({0, 0, 100, 100}, paint);
    paint.setShader(nullptr);
    recordingCanvas->drawString("Blend", 0, 125, font, black);
    
    // Opaque color (0xFFBCBCBC)
    recordingCanvas->translate(105, 0);
    paint.setColor(0xFFBCBCBC);
    recordingCanvas->drawRect({0, 0, 100, 100}, paint);
    recordingCanvas->drawString("0xFFBCBCBC", 0, 125, font, paint);
    
    // Opaque color (0xFF808080)
    recordingCanvas->translate(105, 0);
    paint.setColor(0xFF808080);
    recordingCanvas->drawRect({0, 0, 100, 100}, paint);
    recordingCanvas->drawString("0xFF808080", 0, 125, font, paint);
