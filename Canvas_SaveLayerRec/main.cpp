...
void draw(SkCanvas* canvas) {
    ...
    SkMatrix matrix;
    matrix.setScale(4, 4);
    SkRect bounds = SkRect::MakeLTRB(125.0f, 85.0f, 256.0f, 100.0f);
    auto scaler = SkImageFilters::MatrixTransform(matrix, SkSamplingOptions(), nullptr);
    // 前面绘制的两个圆形被 scale 放大了四倍
    // 为什么这里的 bounds 没有任何的作用
    SkCanvas::SaveLayerRec saveLayerRec(&bounds, nullptr, scaler.get(), 0);
    canvas->saveLayer(saveLayerRec);
    // 这里的绘制不会被放大
    canvas->drawCircle(125, 85, 8, redPaint);
    canvas->restore();
    canvas->drawCircle(141, 85, 8, redPaint);
}
...
void draw1(SkCanvas* canvas) {
    ...
    canvas->drawCircle(125, 85, 8, redPaint);
    SkMatrix matrix1;
    matrix1.setScale(.25f, .25f);
    auto scaler1 = SkImageFilters::MatrixTransform(matrix1, SkSamplingOptions(), nullptr);
    SkCanvas::SaveLayerRec saveLayerRec1(nullptr, nullptr, scaler1.get(), 0);
    canvas->saveLayer(saveLayerRec1);
    canvas->restore();
}
