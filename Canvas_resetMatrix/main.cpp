...
// https://fiddle.skia.org/c/@Canvas_resetMatrix
void draw(SkCanvas* canvas) {
    SkPaint paint;
    SkFont font = SkFont(typeFace);
    canvas->scale(4, 6);
    canvas->drawString("truth", 2, 10, font, paint);
    SkMatrix totalMatrix = canvas->getTotalMatrix();
    printSkMatrix(totalMatrix, "before reset");
    canvas->resetMatrix();
    totalMatrix = canvas->getTotalMatrix();
    printSkMatrix(totalMatrix, "after reset");
    canvas->scale(2.8f, 6);
    totalMatrix = canvas->getTotalMatrix();
    printSkMatrix(totalMatrix, "scale second time");
