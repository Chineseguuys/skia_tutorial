...
// shift pre pixels
// 返回每个像素所占内存的字节数以 2 为底的对数值。其设计目的是为了优化像素地址计算时的性能，通过位移运算代替乘法运算。
void draw(SkCanvas* canvas) {
    std::map<int32_t, std::string> SkColorType2Str = {
        ...
        {kRGB_888x_SkColorType, "kRGB_888x_SkColorType"},
        {kBGRA_8888_SkColorType, "kBGRA_8888_SkColorType"},
        {kRGBA_1010102_SkColorType, "kRGBA_1010102_SkColorType"},
        {kRGB_101010x_SkColorType, "kRGB_101010x_SkColorType"},
        {kGray_8_SkColorType, "kGray_8_SkColorType"},
        {kRGBA_F16Norm_SkColorType, "kRGBA_F16Norm_SkColorType"},
    ...
    kGray_8_SkColorType, kRGBA_F16Norm_SkColorType, kRGBA_F16_SkColorType
        ...
        spdlog::info("color: {:<30}shiftPerPixel:{:<2}, bytesPrePixel:{}",
                SkColorType2Str[colorType], bitmap.shiftPerPixel(), bitmap.bytesPerPixel());
    }
}
...
void draw1(SkCanvas* canvas) {
    ...
    bitmap.setInfo(SkImageInfo::Make(4, 4, kRGB_565_SkColorType, kPremul_SkAlphaType));
    SkDebugf("rowBytes:%zu, rowBytesAsPixels:%zu\n", bitmap.rowBytes(), bitmap.rowBytesAsPixels());
}
