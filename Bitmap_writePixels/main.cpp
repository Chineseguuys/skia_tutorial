...
void draw(SkCanvas* canvas) {
    ...
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            ...
            // 负值意味着，在拷贝到目标位置的时候，不考虑 dstX 和 dstY 的值，永远从 (0, 0) 的位置写入
            // 当然读取的时候，从 source 的 abs(dstX), abs(dstY) 的位置开始读取
